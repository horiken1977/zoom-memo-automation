  ...
  extractClientName(meetingInfo) {
    ...
    if (meetingInfo.structuredSummary && meetingInfo.structuredSummary.clientName) {
      ...
      if (aiClientName && aiClientName !== '不明' && aiClientName !== 'N/A') {
        logger.info(`Document storage - Using AI-extracted client name: "${aiClientName}" (from structured summary)`);
        return aiClientName;
      }
    }
    ...
    if (meetingInfo.summary && meetingInfo.summary.client) {
      ...
      if (aiClientName && aiClientName !== '不明' && aiClientName !== 'N/A') {
        logger.info(`Document storage - Using AI-extracted client name: "${aiClientName}" (from legacy summary)`);
        return aiClientName;
      }
    }
      ...
      if (parts.length > 1 && parts[0]) {
        ...
        if (clientName.length >= 2) {
          ...
          if (!excludeWords.includes(clientName)) {
            logger.info(`Document storage - Extracted client name from topic: "${clientName}" (from: "${meetingInfo.topic}")`);
            return clientName;
          }
        }
      }
    ...
    logger.info(`Document storage - Using fallback folder name: "${fallbackName}" (no client name found)`);
    return fallbackName;
  }
